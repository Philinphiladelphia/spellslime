...
/*
	There are two "string" classes: ByteString and PTString. They have nearly
	identical interfaces, except that one stores 8-bit octets (bytes) and
	the other stores Unicode codepoints.

	For ease of use, a PTString can be constructed from a string literal
	...
	Insert(size_t pos, PTString str)
	...
	BeginsWith(PTString prefix)
	EndsWith(PTString suffix)
	Contains(PTString infix)
	...
	PTString::Build methods respectively. PTString::Build(x, y, z) is roughly
	equivalent to:
		StringBuilder tmp;
		tmp << x << y << z;
		return tmp.Build();
	...
	or failed. The idiomatic code goes like:
		if(PTString::Split split = str.SplitBy(','))
	...
	SplitBy(PTString sep, size_t from = 0)
	...
	SplitByAny(PTString chars, size_t from = 0)
		Split on the first occurence of any of the characters in
		<chars>.

	SplitByNot(PTString chars, size_t from = 0)
		Split on the first occurence of any character that is *not* in
		<chars>.

	SplitFromEndBy(PTString sep, size_t from = npos)
	SplitFromEndBy(value_type sep, size_t from = npos)
	SplitFromEndByAny(PTString chars, size_t from = npos)
	SplitFromEndByNot(PTString chars, size_t from = npos)
	...
	It is recommented to use ByteString::value_type and PTString::value_type
	...
	more space-efficient UTF-8). Therefore the PTString class is derived from
	...
	have to implement our own (See "common/PTString.cpp").
*/
...
public:
	...
	PTString FromUtf8(bool ignoreError = true) const;
	inline PTString FromAscii() const;
...
class PTString : public std::basic_string<char32_t>
{
	using super = std::basic_string<char32_t>;
public:
	inline PTString(): super() {}
	inline PTString(value_type ch): super(1, ch) {}
	inline PTString(size_type count, value_type ch): super(count, ch) {}
	inline PTString(value_type const *ch, size_type count): super(ch, count) {}
	inline PTString(value_type const *ch): super(ch) {}
	template<class It> inline PTString(It first, It last): super(first, last) {}
	inline PTString(super const &other): super(other) {}
	inline PTString(super &&other): super(std::move(other)) {}
	inline PTString(PTString const &other): super(other) {}
	inline PTString(PTString &&other): super(std::move(other)) {}
	template<size_t N> inline PTString(ByteString::value_type const (&ch)[N]): super(ByteString(ch, N - 1).FromAscii()) {}

	inline PTString &operator=(PTString const &other) { super::operator=(other); return *this; }
	inline PTString &operator=(PTString &&other) { super::operator=(other); return *this; }

	inline PTString &operator+=(PTString const &other) { super::operator+=(other); return *this; }
	inline PTString &operator+=(PTString &&other) { super::operator+=(std::move(other)); return *this; }

	inline PTString Substr(size_t pos = 0, size_t count = npos) const { return super::substr(pos, count); }
	inline PTString SubstrFromEnd(size_t rpos = 0, size_t rcount = npos) const { return super::substr(rcount == npos || rcount > rpos ? 0 : rpos - rcount, size() - rpos); }
	inline PTString Between(size_t from, size_t to) const { return to == npos ? super::substr(from) : from >= to ? PTString() : super::substr(from, to - from); }

	inline bool Contains(value_type ch) const { return super::find(ch) != npos; }
	inline bool Contains(PTString const &other) const { return super::find(other) != npos; }

	inline bool BeginsWith(PTString const &other) const { return !super::compare(0, other.size(), other); }
	inline bool EndsWith(PTString const &other) const { return size() >= other.size() && !super::compare(size() - other.size(), other.size(), other); }

	using Split = SplitBase<PTString>;
	inline Split SplitBy(value_type ch, size_t pos = 0) const { return Split(*this, pos, super::find(ch, pos), 1, false); }
	inline Split SplitBy(PTString const &str, size_t pos = 0) const { return Split(*this, pos, super::find(str, pos), str.size(), false); }
	inline Split SplitByAny(PTString const &str, size_t pos = 0) const { return Split(*this, pos, super::find_first_of(str, pos), 1, false); }
	inline Split SplitByNot(PTString const &str, size_t pos = 0) const { return Split(*this, pos, super::find_first_not_of(str, pos), 1, false); }
	inline Split SplitFromEndBy(value_type ch, size_t pos = npos) const { return Split(*this, pos, super::rfind(ch, pos), 1, true); }
	inline Split SplitFromEndBy(PTString const &str, size_t pos = npos) const { return Split(*this, pos, super::rfind(str, pos), str.size(), true); }
	inline Split SplitFromEndByAny(PTString const &str, size_t pos = npos) const { return Split(*this, pos, super::find_last_of(str, pos), 1, true); }
	inline Split SplitFromEndByNot(PTString const &str, size_t pos = npos) const { return Split(*this, pos, super::find_last_not_of(str, pos), 1, true); }
private:
	Split SplitSigned(long long int &, size_t, std::ios_base::fmtflags, std::ios_base::fmtflags) const;
	Split SplitUnsigned(unsigned long long int &, size_t, std::ios_base::fmtflags, std::ios_base::fmtflags) const;
	Split SplitFloat(double &, size_t, std::ios_base::fmtflags, std::ios_base::fmtflags) const;
public:
	...
	std::vector<PTString> PartitionBy(value_type ch, bool includeEmpty = false) const;
	std::vector<PTString> PartitionBy(PTString const &str, bool includeEmpty = false) const;
	std::vector<PTString> PartitionByAny(PTString const &str, bool includeEmpty = false) const;

	PTString &Substitute(PTString const &needle, PTString const &replacement);

	inline PTString &Insert(size_t pos, PTString const &str) { super::insert(pos, str); return *this; }
	inline PTString &Erase(size_t pos, size_t count) { super::erase(pos, count); return *this; }
	inline PTString &EraseBetween(size_t from, size_t to) { if(from < to) super::erase(from, to - from); return *this; }

	inline PTString ToLower() const
	{
		std::locale const &loc = std::locale::classic();
		PTString value(*this);
		for(value_type &ch : value)
	...
	template<typename... Ts> static PTString Build(Ts&&... args);
};

inline PTString operator+(PTString const &lhs, PTString const &rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(PTString const &lhs, PTString &&rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + static_cast<std::basic_string<char32_t> &&>(rhs); }
inline PTString operator+(PTString &&lhs, PTString const &rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(PTString &&lhs, PTString &&rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + static_cast<std::basic_string<char32_t> &&>(rhs); }
inline PTString operator+(PTString const &lhs, std::basic_string<char32_t> const &rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + rhs; }
inline PTString operator+(PTString const &lhs, std::basic_string<char32_t> &&rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + std::move(rhs); }
inline PTString operator+(PTString &&lhs, std::basic_string<char32_t> const &rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + rhs; }
inline PTString operator+(PTString &&lhs, std::basic_string<char32_t> &&rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + std::move(rhs); }
inline PTString operator+(PTString const &lhs, PTString::value_type rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + rhs; }
inline PTString operator+(PTString &&lhs, PTString::value_type rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + rhs; }
inline PTString operator+(PTString const &lhs, PTString::value_type const *rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) + rhs; }
inline PTString operator+(PTString &&lhs, PTString::value_type const *rhs) { return static_cast<std::basic_string<char32_t> &&>(lhs) + rhs; }
inline PTString operator+(std::basic_string<char32_t> const &lhs, PTString const &rhs) { return lhs + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(std::basic_string<char32_t> const &lhs, PTString &&rhs) { return lhs + static_cast<std::basic_string<char32_t> &&>(rhs); }
inline PTString operator+(std::basic_string<char32_t> &&lhs, PTString const &rhs) { return std::move(lhs) + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(std::basic_string<char32_t> &&lhs, PTString &&rhs) { return std::move(lhs) + static_cast<std::basic_string<char32_t> &&>(rhs); }
inline PTString operator+(PTString::value_type lhs, PTString const &rhs) { return lhs + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(PTString::value_type lhs, PTString &&rhs) { return lhs + static_cast<std::basic_string<char32_t> &&>(rhs); }
inline PTString operator+(PTString::value_type const *lhs, PTString const &rhs) { return lhs + static_cast<std::basic_string<char32_t> const &>(rhs); }
inline PTString operator+(PTString::value_type const *lhs, PTString &&rhs) { return lhs + static_cast<std::basic_string<char32_t> &&>(rhs); }
template<size_t N> inline PTString operator+(PTString const &lhs, ByteString::value_type const (&rhs)[N]) { return static_cast<std::basic_string<char32_t> const &>(lhs) + ByteString(rhs).FromAscii(); }
template<size_t N> inline PTString operator+(PTString &&lhs, ByteString::value_type const (&rhs)[N]) { return static_cast<std::basic_string<char32_t> &&>(lhs) + ByteString(rhs).FromAscii(); }
template<size_t N> inline PTString operator+(ByteString::value_type const (&lhs)[N], PTString const &rhs) { return ByteString(lhs).FromAscii() + static_cast<std::basic_string<char32_t> const &>(rhs); }
template<size_t N> inline PTString operator+(ByteString::value_type const (&lhs)[N], PTString &&rhs) { return ByteString(lhs).FromAscii() + static_cast<std::basic_string<char32_t> &&>(rhs); }

inline bool operator==(PTString const &lhs, PTString const &rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) == static_cast<std::basic_string<char32_t> const &>(rhs); }
inline bool operator==(PTString const &lhs, std::basic_string<char32_t> const &rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) == rhs; }
inline bool operator==(PTString const &lhs, PTString::value_type const *rhs) { return static_cast<std::basic_string<char32_t> const &>(lhs) == rhs; }
inline bool operator==(std::basic_string<char32_t> const &lhs, PTString const &rhs) { return lhs == static_cast<std::basic_string<char32_t> const &>(rhs); }
inline bool operator==(PTString::value_type const *lhs, PTString const &rhs) { return lhs == static_cast<std::basic_string<char32_t> const &>(rhs); }
template<size_t N> inline bool operator==(PTString const &lhs, ByteString::value_type const (&rhs)[N]) { return static_cast<std::basic_string<char32_t> const &>(lhs) == ByteString(rhs).FromAscii(); }
template<size_t N> inline bool operator==(ByteString::value_type const (&lhs)[N], PTString const &rhs) { return ByteString(lhs).FromAscii() == static_cast<std::basic_string<char32_t> const &>(rhs); }
...
inline PTString ByteString::FromAscii() const
{
	PTString destination = PTString(size(), PTString::value_type());
	for(size_t i = 0; i < size(); i++)
		destination[i] = PTString::value_type(std::make_unsigned<ByteString::value_type>::type(operator[](i)));
	return destination;
}

inline ByteString PTString::ToAscii() const
{
...
class StringBuilder
{
	std::vector<PTString::value_type> buffer;
public:
	std::ios_base::fmtflags flags;
	...
	void AddChars(PTString::value_type const *, size_t);
	size_t Size() const { return buffer.size(); }
	PTString Build() const;
...
StringBuilder &operator<<(StringBuilder &, PTString::value_type);
StringBuilder &operator<<(StringBuilder &, PTString::value_type const *);
StringBuilder &operator<<(StringBuilder &, PTString const &);
...
template<typename... Ts> PTString PTString::Build(Ts&&... args)
{
	StringBuilder b;
	b.Add(std::forward<Ts>(args)...);
	return b.Build();
